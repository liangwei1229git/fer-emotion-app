...
class FERMultiModel:
        ...
        self.photo_tips = [
            {
                'icon': '💡',
                'title': '光线充足',
                'description': '确保面部光线明亮均匀，避免逆光或阴影'
            },
            {
                'icon': '📷',
                'title': '正面朝向',
                'description': '面部正对摄像头，不要侧面太多'
            },
            {
                'icon': '📏',
                'title': '合适距离',
                'description': '让面部占据画面的40-60%左右'
            },
            {
                'icon': '😊',
                'title': '自然表情',
                'description': '表情要自然放松，不要太刻意'
            },
            {
                'icon': '🖼️',
                'title': '简洁背景',
                'description': '背景尽量简洁，避免杂乱'
            }
        ]
        
        print("FER Multi-Model initialized successfully!")
    ...
    def predict(self, image, detector_backend='retinaface'):
        try:
...
fer_model = FERMultiModel()

@app.route('/')
def index():
    return render_template('index_multi_model.html')
...
def predict():
    try:
        ...
        result = fer_model.predict(image, detector_backend)
